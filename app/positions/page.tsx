...
export default function Positions() {
			...
			for (let i = 0; i < totalMarkets; i++) {
				try {
					const response = await fetch(
						`/api/user-position?marketId=${i}&address=${address}`
					);
					if (response.ok) {
						const data = await response.json();
						if (
							data.upShares > BigInt(0) ||
							data.downShares > BigInt(0)
						) {
	...
	const handleSell = async (
		...
		setPendingTransaction({ type: "sell", marketId, outcome, shares });
		await simulate({
			type: "sellShares",
			marketId,
			outcome,
	...
	const handleClaimWinnings = async (marketId: bigint) => {
		setPendingTransaction({ type: "claim", marketId });
		await simulate({
			type: "claimWinnings",
			marketId
		});
	...
	const executeSellTransaction = async () => {
		if (!pendingTransaction || pendingTransaction.type !== "sell") return;
		...
		try {
			await writeContract({
				...
				args: [
					pendingTransaction.marketId,
					pendingTransaction.outcome!,
					pendingTransaction.shares!
				]
			});
			clearSimulation();
	...
	const executeClaimTransaction = async () => {
		if (!pendingTransaction || pendingTransaction.type !== "claim") return;
		...
		try {
			await writeContract({
				...
				args: [pendingTransaction.marketId]
			});
			clearSimulation();
			setPendingTransaction(null);
		} catch (error) {
							...
							{positions.map((position, index) => {
								const hasWon =
									...
									((position.winningOutcome === 0 &&
										position.upShares > BigInt(0)) ||
										(position.winningOutcome === 1 &&
											position.downShares > BigInt(0)));
